...
/**
 * Backpropagation Perceptron network example - is data from the Iris Dataset. Recognize 3 classes.
 */
public class IrisDataSetApp {
    private static final float LEARNING_RATE = 0.3f;
    private static final float MOMENTUM = 0.6f;
    private static final int ITERATIONS = 200_000;
    //Move training data to the bottom of file
    private static final float[][] trainingResults = new float[][]{
            //3x50 results, 50 per each class
            new float[]{1f, 0f, 0f},
            new float[]{1f, 0f, 0f},
            new float[]{1f, 0f, 0f},
            new float[]{1f, 0f, 0f},
    ...
    private static final float[][] trainingData = new float[][]{
    ...
    public static void main(String[] args) {
        BackpropNeuralNetwork network = new BackpropNeuralNetwork(4, 6, 3);

        for (int iteration = 1; iteration <= ITERATIONS; iteration++) {

            for (int i = 0; i < trainingResults.length; i++) {
                network.train(trainingData[i], trainingResults[i], LEARNING_RATE, MOMENTUM);
            }

            if ((iteration) % 20_000 == 0 || iteration == 1) {
                System.out.printf("Iteration # %d \n", iteration);
                for (int i = 0; i < trainingResults.length; i++) {
                    float[] data = trainingData[i];
                    float[] calculatedOutput = network.run(data);
