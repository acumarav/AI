...
public class IrisDataset {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < nEpochs; i++) {
            neuralNetwork.fit(trainDataset);
        }

        //we evaluate the model on the testDataset set
        Evaluation evaluation = new Evaluation(numOutput);
        evaluation.eval(testDataset.getLabels(), neuralNetwork.output(testDataset.getFeatures(), Layer.TrainingMode.TEST));
